...
> `victoryCapital = 5.000` trong code. Thanh vốn trên HUD dùng `capitalHudMax = 20.000` chỉ để hiển thị %.

### 5.3. Chiến thắng đặc biệt (câu 15)

Câu **Chốt Lời Lịch Sử** — chọn **Bán cho quỹ** với vốn `>= 5.000` → `forcedPhase: victory` với ending riêng (xem mục 8).
...
|-----|-----|----------|--------|
| 15 | `historic-exit` | **Trái — Không bán** | Sau câu 15: nếu vốn `< 5.000` → game over với `Game kết thúc: doanh nghiệp chưa đạt đủ 5.000 vốn tích lũy để được xem là chiến thắng.` |
...
### 7.4. Hết 15 câu mà vốn < 5.000

Mọi lối chơi không đạt 5.000 vốn đều **game over** khi hoàn thành lượt 15 — kể cả chọn **Không bán** ở câu cuối. Ví dụ: vốn 6.000 + **Không bán** → **VICTORY** (điểm thấp trên bảng xếp hạng).
...
|------------|-----------|
| **Vượt ải** | Hoàn thành 15 câu, vốn `>= 5.000`, không rơi vào critical failure |
| **Bán công ty** (câu 15) | Chọn **Bán cho quỹ đầu tư**, vốn `>= 5.000` → `forcedPhase: victory` (ending riêng) |
...
| Kết quả | Hết ván: vốn ≥ 5.000 → thắng; `< 5.000` → thua | Đủ vốn → **VICTORY** (ending bán công ty); thiếu vốn → thua |
...
**Ngưỡng thắng trong code:** vốn **≥ 5.000** sau 15 câu (hoặc bán công ty ở câu 15 với cùng ngưỡng). Điểm trên bảng xếp hạng phản ánh vốn cuối — thắng với 6.000 vẫn là VICTORY nhưng xếp dưới người có vốn cao hơn.
...
| 5 | Chốt Lời Lịch Sử | Không bán, hết 15 câu, vốn < 5.000 | `resolveGamePhase` + `gameOverEnding` |

**Game over gián tiếp phổ biến:** vốn ≤ 0, uy tín < 1 (đặc biệt nhập sữa lậu), sức lao động < 1 (xếp chồng bóc lột), hoặc hết 15 câu không đủ 5.000 vốn.
