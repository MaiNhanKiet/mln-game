...
const RANK_COLUMN_CLASS = 'w-11 shrink-0'
const STATUS_BADGE_COLUMN_CLASS = 'w-[4.75rem] shrink-0'
...
export function Leaderboard({ state, onRestart }: LeaderboardProps) {
  ...
  return (
    <>
    <section className="min-h-0 flex-1 overflow-y-auto overscroll-contain w-full rounded-game bg-background p-2 [-webkit-overflow-scrolling:touch] sm:p-3">
      <div className="text-center">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border-2 border-secondary bg-white shadow-[3px_3px_0_0_#42362E]">
          <Trophy size={24} strokeWidth={2.8} />
        </div>
        <h2 className="mt-3 font-display text-3xl font-extrabold leading-none text-secondary">
          Tổng kết hành trình
        </h2>
        <p className="mt-2 text-xs font-semibold whitespace-nowrap text-secondary/75 sm:text-sm">
          Điểm cuối ván đã được tự động ghi nhận vào BXH.
        </p>
      </div>

      <div className="mt-4 grid grid-cols-3 gap-2 rounded-game bg-white p-1.5 shadow-control">
        {categories.map((category) => (
          <button
            key={category.id}
            type="button"
            className={`min-h-10 rounded-control px-2 text-xs font-extrabold transition ${
      ...
      <article className="mt-3 min-w-0 rounded-game bg-white p-2 shadow-control sm:mt-4 sm:p-2.5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="truncate whitespace-nowrap font-display text-base font-bold leading-tight text-secondary sm:text-lg">
              {activeConfig.title}
            </h3>
            <p className="mt-1 line-clamp-2 text-[11px] font-semibold leading-4 text-secondary/70 sm:text-xs sm:leading-5">
              {activeConfig.description}
            </p>
          </div>
        </div>

        <div className="mt-3 min-w-0 grid gap-2">
          {activeEntries.length > 0 ? (
            activeEntries.map((entry, index) => {
              ...
              return (
                <div
                  key={entry.id}
                  className={`${LEADERBOARD_ROW_GRID_CLASS} min-h-11 rounded-control border px-2 py-2 text-sm font-extrabold sm:px-2.5 ${getOutcomeRowClass(
                    entry,
      ...
      {state.gameRunId ? (
        <div className="mt-4 rounded-game bg-secondary p-3 text-center text-white shadow-control">
          <p className="text-xs font-extrabold uppercase tracking-wide text-white/65">
            Vị trí của bạn
          </p>
      ...
      {alreadyPlayed ? (
        <p className="mt-4 rounded-game border-2 border-secondary/15 bg-white px-4 py-3 text-center text-sm font-bold text-secondary/70 shadow-control">
          Bạn đã hoàn thành ván chơi. Điểm được giữ trên bảng xếp hạng — mỗi người chỉ chơi một lần.
        </p>
      ) : (
        <button
          type="button"
          className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-game border-2 border-secondary bg-white px-4 py-2 text-sm font-extrabold text-secondary shadow-[3px_3px_0_0_#42362E] transition hover:-translate-y-0.5 active:translate-y-0"
          onClick={onRestart}
        >
      ...
      {expandedPlayerName ? (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 z-[80] w-[min(100vw-1.5rem,20rem)] -translate-x-1/2 overflow-hidden rounded-2xl border border-secondary/15 bg-white shadow-[0_24px_60px_rgba(66,54,46,0.18)]"
            style={{ top: '50%' }}
